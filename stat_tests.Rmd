...
## What is the distribution of number of articles, published in certain channel?

```{r}
unique(data$data_channel_type)
```
```{r}
...
## Significance of negative content
...
par(mfrow = c(1, 1))
```
We will use t-test, to check if mean number of shares could be higher for articles with 
...
```
Unfortunately, Shapiro test shows that even log-transformed data is not normally distributed. 
...
Wilcox (Whitney tests) show, that medians of the subgoupr are statistically different.
