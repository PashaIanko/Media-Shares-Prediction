...
```{r}
standardized_train = standardize_data(
  train,
  'shares'
)

standardized_test = standardize_data(
  test,
  'shares'
)
```
...
X = standardized_train[, -length(colnames(data))]
Y = standardized_train$shares

grid <- 10^seq(10, -2, length=100)
ridge.mod <- glmnet(
  X, # data[, -length(colnames(data))],
  Y, # data$shares,
  alpha = 0,
  lambda = grid,
  standardize = F
...
set.seed(1)
cv.out <- cv.glmnet(
  as.matrix(standardized_train[, -length(colnames(standardized_train))]), 
  standardized_train[, 'shares'], 
  alpha = 0, 
  nfold=10
)
...
ridge.pred <- predict(
  ridge.mod, 
  s = bestlam,
  newx = as.matrix(standardized_test[, -length(colnames(standardized_test))])
)

y.test = standardized_test$shares
mean((ridge.pred - y.test)^2)
```
```{r}
print(ridge.pred[1:10])
print(y.test[1:10])
```
