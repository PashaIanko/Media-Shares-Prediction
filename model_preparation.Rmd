...
```
```{r}
bic_forward_coefs = names(coefficients(forward_subsets, n_best_bic_fwd))[-1]
stopifnot(length(bic_forward_coefs) == n_best_bic_fwd)

bic_backward_coefs = names(coefficients(backward_subsets, n_best_bic_back))[-1]
stopifnot(length(bic_backward_coefs) == n_best_bic_back)

cp_forward_coefs = names(coefficients(forward_subsets, n_best_cp_fwd))[-1]
stopifnot(length(cp_forward_coefs) == n_best_cp_fwd)

cp_backward_coefs = names(coefficients(backward_subsets, n_best_cp_back))[-1]
stopifnot(length(cp_backward_coefs) == n_best_cp_back)
...
Up to now, from lectures we know that backward model selection is more preferrable than the forward. According to the backward procedure, we can highlight two models - with 19 parameters (minimum of CP criteria) and 7 parameters (Minimum of Bayesian Information Criteria). These models will be further tested



```{r}
coef(forward_subsets, 7)
```
