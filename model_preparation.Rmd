...
```{r}
data <- read.csv('C://Users//c//OneDrive//Bureau//Media-Shares-Prediction//OnlineNewsPopularity_processed.csv')
print(dim(data))
head(data)
```
...
#SVM
```{r}
#split data into training set and training set 

intrain <- createDataPartition(y = data$shares, p= 0.7, list = FALSE)
...
```{r}
training[["shares"]] = factor(training[["shares"]])
```
...
```{r}
svm_Linear <- train(shares ~., data = training, method = "svmLinear", trControl=trctrl,preProcess = c("center", "scale"), tuneLength = 10)
```
...
```{r}
confusionMatrix(table(test_pred, testing$shares))
```
