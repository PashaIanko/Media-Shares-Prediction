...
```{r}
Q1 <- quantile(data$num_hrefs, .25)
Q3 <- quantile(data$num_hrefs, .75)
IQR <- IQR(data$num_hrefs)
print(IQR)

no_outliers <- subset(data, data$num_hrefs > (Q1 - 1.5*IQR) & data$num_hrefs < (Q3 + 1.5*IQR))
dim(no_outliers)
dim(data)
```
